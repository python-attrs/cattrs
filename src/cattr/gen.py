...
def make_dict_structure_fn(
    ...
    **kwargs,
    ...
    if _cattrs_extended_validation:
        globs["ClassValidationError"] = ClassValidationError
    lines = []
    post_lines = []
    ...
    if _cattrs_extended_validation:
        # Even though the validation path and the non-validation
        # path are similar, we duplicate the code anyway for ease
        # of understanding and divergence.
        lines.append("  res = {}")
        lines.append("  errors = {}")
        allowed_fields = set()
        for a in attrs:
            ...
            if a.default is NOTHING:
                if handler:
                    ...
                    lines.append(f"    res['{ian}'] = o['{kn}']")
            else:
                lines.append(f"    if '{kn}' in o:")
                if handler:
                    lines.append(
                        f"      res['{ian}'] = {struct_handler_name}(o['{kn}'], type_{an})"
                    )
                    ...
                    lines.append(f"      res['{ian}'] = o['{kn}']")
            lines.append(f"  except Exception as e: errors['{an}'] = e")
        post_lines.append("  if errors: raise ClassValidationError(errors)")
    else:
